...
print(fc1_w.shape)  # [128,1024]
print(fc1_b.shape)  #[128]

fc2_w = state["fc2.weight"]
fc2_b = state["fc2.bias"]
print(fc2_w.shape)  # [10,128]
print(fc2_b.shape)  #[10]

# save all those weights and bias
# MPSCNN requires weights to be [oC kH kW iC] -> NHWC
# PyTorch is NCHW
np.savetxt('./params/conv1_W.txt', [conv1_w.view(-1).numpy()], delimiter=',')
np.savetxt('./params/conv1_b.txt', [conv1_b.numpy()], delimiter=',')
np.savetxt('./mps/conv1_W.txt',
           [conv1_w.permute(0, 2, 3, 1).contiguous().view(-1).numpy()],
           delimiter=',')
np.savetxt('./mps/conv1_b.txt', [conv1_b.numpy()], delimiter=',')

np.savetxt('./params/conv2_W.txt', [conv2_w.view(-1).numpy()], delimiter=',')
np.savetxt('./params/conv2_b.txt', [conv2_b.numpy()], delimiter=',')
np.savetxt('./mps/conv2_W.txt',
           [conv2_w.permute(0, 2, 3, 1).contiguous().view(-1).numpy()],
           delimiter=',')
np.savetxt('./mps/conv2_b.txt', [conv2_b.numpy()], delimiter=',')

np.savetxt('./params/conv3_W.txt', [conv3_w.view(-1).numpy()], delimiter=',')
np.savetxt('./params/conv3_b.txt', [conv3_b.numpy()], delimiter=',')
np.savetxt('./mps/conv3_W.txt',
           [conv3_w.permute(0, 2, 3, 1).contiguous().view(-1).numpy()],
           delimiter=',')
np.savetxt('./mps/conv3_b.txt', [conv3_b.numpy()], delimiter=',')

# np.savetxt('./mps/fc1_W.txt', [fc1_w.view(-1).numpy()], delimiter=',')
np.savetxt('./mps/fc1_W.txt', [
    fc1_w.view(128, 64, 4, 4).permute(0, 2, 3,
                                      1).contiguous().view(-1).numpy()
],
           delimiter=',')
np.savetxt('./mps/fc1_b.txt', [fc1_b.numpy()], delimiter=',')
np.savetxt('./mps/fc2_W.txt', [
    fc2_w.view(10, 128, 1, 1).permute(0, 2, 3,
                                      1).contiguous().view(-1).numpy()
],
           delimiter=',')
np.savetxt('./mps/fc2_b.txt', [fc2_b.numpy()], delimiter=',')
